...
export const fetchPokemon = async (name: string, test?: boolean) => {
	try {
		...
		if (response.ok) {
			...
			const specialEvolutionOfPokemonData =
				specialEvoOfPokemonData.length > 0
					? await fetchPokemonEvolutionData(specialEvoOfPokemonData)
					: undefined
			const pokemon: pokemonType = {
				...
				stats: data.stats.map(
					(stat: { stat: { name: string }; base_stat: number }) => {
						return { number: stat.base_stat, name: stat.stat.name }
					}
				),
